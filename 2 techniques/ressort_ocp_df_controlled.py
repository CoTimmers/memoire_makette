"""Optimal Control Problem — Rockit / IPOPT
   ...
   Par rapport à la version fx/fy :
     - fx et fy deviennent des ÉTATS avec set_der(fx, dfx)
     - dfx, dfy sont les CONTRÔLES (forces lisses garanties)
     - Pas de condition initiale dure sur fx(0), fy(0)
       → le solver est libre d'appliquer des forces dès t=0
     - Régularisation sur dfx²+dfy² (lissage) plutôt que fx²+fy²
     - Warm start cohérent : dfx_init = gradient(fx_init)
"""
...
T_sim      = 10.0
F_max      = 40.0
dF_max     = 20.0          # limite taux de variation N/s
PSI_SWITCH = 140.0 * np.pi / 180
T_RAMP     = 5.0

# ══════════════════════════════════════════════════════════════════════
# DYNAMIQUE  (identique à la version fx/fy)
# ══════════════════════════════════════════════════════════════════════
...
psidot    = ocp.state()
tau_clock = ocp.state()
fx        = ocp.state()    # ← état (intégrale de dfx)
fy        = ocp.state()    # ← état (intégrale de dfy)

# ── Contrôles ─────────────────────────────────────────────────────────
dfx = ocp.control()        # ← taux de variation de fx
dfy = ocp.control()        # ← taux de variation de fy
...
# gate_started ≈ 1 dès que tau_clock > 0.1 (ne retombe jamais car tau_clock croissant)
...
ocp.set_der(fx,        dfx)     # ← intégration des contrôles
ocp.set_der(fy,        dfy)
...
# ══════════════════════════════════════════════════════════════════════
W_track = 1e4
W_u     = 1e-2   # régularisation sur dfx/dfy (lissage forces)
W_c     = 1e2
W_effort = 1e4
W_time = 1e4
ocp.add_objective(W_track * ocp.integral((l - l_ref)**2))
ocp.add_objective(W_effort * ocp.integral(fx**2 + fy**2))  
ocp.add_objective(W_u     * ocp.integral(dfx**2 + dfy**2))   # ← lissage
...
# Taux de variation bornés (réalisme physique)
...
# fx(0) et fy(0) LIBRES → pas de subject_to ici
# → le solver peut appliquer des forces dès t=0 pour ouvrir le mur
...
tau_clock_init = np.where(tau <= t_sw, 0.0, (tau - t_sw) * T_sim)

dt_ocp      = T_sim / N
psidot_init = np.gradient(psi_init, dt_ocp)
ldot_init   = np.maximum(np.gradient(l_init, dt_ocp), 0.0)

# Warm start fx/fy (états) : profil physiquement raisonnable
F_push        = 15.0
...
# Warm start dfx/dfy (contrôles) : gradient de fx_init/fy_init
# → cohérence état/contrôle indispensable pour IPOPT
...
ocp.set_initial(fx,        fx_init_state)   # ← état
ocp.set_initial(fy,        fy_init_state)   # ← état
ocp.set_initial(dfx,       dfx_init)        # ← contrôle
ocp.set_initial(dfy,       dfy_init)        # ← contrôle
...
ax.set_title('l(t) vs l_ref'); ax.set_ylabel('l (m)'); ax.set_xlabel('t (s)')
...
print(f"final \nl          = {l_sol[-1]:.4f} m  (target = {b} m)")
print(f"final ldot          = {ldot_sol[-1]:.4f} m/s")
print(f"final ψ           = {np.degrees(psi_sol[-1]):.2f}°")
print(f"final τ_clock    = {tau_clock_sol[-1]:.2f} s")
...
ax2.set_xlim(-0.65, 0.65);  ax2.set_ylim(-0.10, 0.65)
ax2.set_aspect('equal');  ax2.grid(True, alpha=0.3)
ax2.set_xlabel('x (m)');  ax2.set_ylabel('y (m)')
ax2.set_title('OCP simulation')
