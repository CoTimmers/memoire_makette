...
plt.title(f"Applied forces (Fn_mean={np.mean(Fn_opt):.1f}N)")
plt.xlabel("Time (s)")
plt.ylabel("Force (N)")
...
plt.step(time_steps, lam_A1, where='post', label='lambda A wall 1', lw=2)
plt.step(time_steps, lam_B1, where='post', label='lambda B wall 1', lw=2)
plt.step(time_steps, lam_A2, where='post', label='lambda A wall 2', lw=2)
plt.step(time_steps, lam_B2, where='post', label='lambda B wall 2', lw=2)

plt.title(f"Reaction forces (Fn_mean={np.mean(Fn_opt):.1f}N, T_total={t_opt:.2f}s)")
plt.xlabel("Time (s)")
