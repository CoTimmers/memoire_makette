...
class KalmanSway:
    ...
    def gonfle(self, facteur: float = 1.0) -> None:
        """Re-open the filter after an event the model does not describe.

        An impact changes theta_dot in a way the propagation cannot predict, so
        the innovation that follows is large and the NIS test would reject the
        very measurements needed to recover. Putting the covariance back to P0
        makes the filter trust the camera again and re-lock within a few frames,
        without throwing away the current estimate the way a full reset would.
        """
        self.P = facteur * self.P0.copy()
        for h in self.histo:
            h["P"] = self.P.copy()
...
class SwayEstimator2D:
    ...
    def update(self, theta_mes, t_mes: float,
               seuil_nis: float = 25.0) -> tuple[bool, bool]:
        return (self.x.update(float(theta_mes[0]), t_mes, seuil_nis),
                self.y.update(float(theta_mes[1]), t_mes, seuil_nis))

    def gonfle(self, facteur: float = 1.0) -> None:
        self.x.gonfle(facteur)
        self.y.gonfle(facteur)
