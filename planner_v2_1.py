...
def plan(
    ...
    Compute the time-optimal trajectory from x0 to xf under:
        - v   <= vmax       (hard kinematic limit)
        - a   <= amax       (hard kinematic limit)
        - a   <= a_theta_max  (swing angle limit)
        - ta_accel = ta_decel = ta_star = Td/2  (phase cancellation)
        - tc  = k * Td/2  for k = 0, 1, 2, ...  (exact anti-oscillation)

    The last two constraints together guarantee zero residual oscillation.
    tc is rounded UP to the nearest multiple of Td/2 to satisfy the distance.
    """
    dx = abs(xf - x0)
    if dx < 1e-9:
    ...
    v_opt     = min(limits.vmax, a_allowed * ta_star)
    a_opt     = v_opt / ta_star
    dx_ramps  = v_opt * ta_star   # distance covered by accel + decel ramps

    if dx <= dx_ramps:
        # --- Triangular profile ---
        # tc = 0  (k=0), reduce a to fit the distance exactly.
        # a_tri <= a_allowed guaranteed since dx <= dx_ramps = a_allowed * ta_star²
        a_tri = dx / ta_star**2
        v_tri = a_tri * ta_star
        ...
        # --- Trapezoidal profile ---
        # tc must be an ODD multiple of Td/2 for exact oscillation cancellation:
        #   tc = (2m+1) * Td/2  for m = 0, 1, 2, ...
        # i.e. tc = Td/2, 3*Td/2, 5*Td/2, ...
        #
        # For a given tc, the distance is: dx = v*(ta_star + tc)
        # So v = dx / (ta_star + tc)
        # We want the smallest odd multiple of Td/2 such that v <= v_opt.
        # v decreases as tc increases, so we find the smallest valid m.
        half_Td = Td / 2.0

        # Minimum tc needed at v_opt (continuous, before quantisation)
        tc_min = (dx - dx_ramps) / v_opt

        # Smallest odd multiple of Td/2 >= tc_min
        m  = math.ceil((tc_min / half_Td - 1.0) / 2.0)
        m  = max(m, 0)
        tc = (2 * m + 1) * half_Td

        # Recompute v to cover exactly dx with this tc
        # dx = v * ta_star + v * tc  =>  v = dx / (ta_star + tc)
        v_adj  = dx / (ta_star + tc)
        a_adj  = v_adj / ta_star

        return TrajectoryProfile(
            type="trapezoidal", x0=x0, xf=xf, direction=direction,
            ta=ta_star, v_peak=direction * v_adj, tc=tc,
            a=direction * a_adj, t_total=2.0 * ta_star + tc,
        )
