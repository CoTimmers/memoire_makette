...
namespace activity_5c
{
    ...
    void ECATActivity::RT_thread()
    {
        ...
        {   
            ...
            if(discrete_state->ETHERCAT_CONFIGURED == 1)
            {
                ...
                {
                    if(ec_receive_processdata(1000) > 0);  // workcounter above 0
                    {
                        ...
                        pthread_mutex_lock(&((&ecx_context)->port->rx_mutex));      // SOEM read mutex
                        clock_gettime(CLOCK_MONOTONIC, continuous_state->RT_ts);     // Update RT time
                        pthread_mutex_unlock(&((&ecx_context)->port->rx_mutex));    // SOEM read mutex
                        
                        discrete_state->ETHERCAT_NEW_DATA = 1;
                    }

                    (*continuous_state->execute_task_function)(this);  // Calculate inputs
                ...
                /* calulate time offset to get linux time and DC synced */
                if (ec_slave[0].hasdc)
                    {time_sync((int64_t) ec_DCtime, (int64_t) params->RT_thread_cycletime, (int64_t*) &time_offset);}
        
                /* Calculate average offset time, if below set value, indicate that drive is synced */
                average_sync_offset = abs(time_offset);         // Reset 
                ...
                sync_offset_ar[ (sizeof(sync_offset_ar)/sizeof(sync_offset_ar[0])) - 1] = time_offset; // include latest sample

                average_sync_offset = average_sync_offset/ (sizeof(sync_offset_ar)/sizeof(sync_offset_ar[0]));
                
                if(average_sync_offset < params->RT_sync_jitter)       // 0.5 microsec
                    {discrete_state->RT_SYNCED = 1;}
                else 
                    {discrete_state->RT_SYNCED = 0;}
            }
            else
                {usleep(2000);}  // Arbitrary sleep to save CPU
        }
    }
     ...
     */
    // void ECATActivity::activity_config(ECATActivity *ecat_activity)
    void ECATActivity::activity_config()
    {
        // activity_t* activity = ecat_activity->activity;
        ...
        case CLEANING:
            break;
        case DONE:
            // TODO add link to deconstructor
            break;
        }
    };
    ...
    void ECATActivity::capability_configuration_coordinate()
    {
        ...
        if(activity->state.lcsm_flags.capability_configuration_complete == true)
        {
            ...
            // if(*activity->state.coordination_state->execution_request)
    ...
    void ECATActivity::running_compute()
    {
        ...
        {
            discrete_state->ETHERCAT_NEW_DATA = 0;  // Handshake

            /* Check which function to execute based on FSM */
            (*continuous_state->execute_FSM)(this);  // Calculate inputs
        }
    }
    ...
    void ECATActivity::running_coordinate()
    {
        if (*coordination_state-> configuration_request){
            activity->lcsm.state = CAPABILITY_CONFIGURATION;
            *coordination_state->configuration_request = false;    // Handshake
        }
        if (*coordination_state->deinitialisation_request){
            activity->state.lcsm_protocol = DEINITIALISATION;
            activity->lcsm.state = RESOURCE_CONFIGURATION;
    ...
    void ECATActivity::configure_from_file(const char *file_path, int *status)
    {
        ...
        param_array_t param_array[16]; // random number 
