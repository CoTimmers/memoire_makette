...
/* FSM to be moved to different file */
    void gantry_FSM(void *in)
    {
        ...
        if(ecat_activity->discrete_state->gantry_state == activity_5c::ECATActivity::GANTRY_HOMING)
            {ecat_activity->continuous_state->execute_task_function = &homing;}
        else if(ecat_activity->discrete_state->gantry_state == activity_5c::ECATActivity::GANTRY_MOVE_TO_ATTACH)
            {ecat_activity->continuous_state->execute_task_function = &move_to_attach;}
        else if(ecat_activity->discrete_state->gantry_state == activity_5c::ECATActivity::GANTRY_WAITING_OPERATOR)
